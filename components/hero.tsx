...
export function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-32 pb-16 md:pb-24">
      ...
      <div className={`absolute inset-0 ${gradientClasses.sectionBackground} ${finalGlow ? "gradient-fade" : ""}`} />
      
      <div className={`${componentClasses.orbsContainer} ${finalGlow ? "gradient-fade" : ""}`}>
      ...
      <div className={`relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center ${sparkActive ? "spark-active" : ""} ${finalGlow ? "final-glow" : ""}`}>
        <div className="space-y-8">
          <div className={`inline-flex items-center space-x-2 ${componentClasses.glassBadge}`}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-8">
            <Link href="/location">
              <Button size="lg" className={`${gradientClasses.buttons.primary} text-white px-8 py-4 text-lg shadow-lg shadow-purple-500/20 md:shadow-purple-500/40`}>
                School Rankings
                <Trophy className="w-5 h-5 ml-2" />
              </Button>
            </Link>
            <Link href="/community">
              <Button size="lg" variant="outline" className={`glass-hover px-8 py-4 text-lg bg-transparent ${gradientClasses.borders.primary} hover:border-purple-500/30 md:hover:border-purple-500/60 ${gradientClasses.buttons.outline}`}>
                Explore Communities
