...
function CustomSelect(props: { options: string[]; value: string; onChange: (v: string) => void; placeholder?: string }) {
  const { options, value, onChange, placeholder } = props
  const [open, setOpen] = useState(false)
  return (
    <div className="relative">
      <button type="button" onClick={() => setOpen(prev => !prev)} className="w-full h-10 flex items-center justify-between px-3 border rounded-md bg-muted/50">
      ...
      {open && (
        <ul className="absolute z-40 mt-1 w-full bg-background border rounded-md max-h-52 overflow-auto">
          {options.map(opt => (
            <li key={opt}>
              <button type="button" onClick={() => { onChange(opt); setOpen(false) }} className="w-full text-left px-3 py-2 hover:bg-muted/30">
                {opt}
              </button>
            </li>
          ))}
...
export default function AuthPage() {
  ...
  const skills = Array.from(new Set(normalizedOpportunities.flatMap(o => o.skills || []).filter((v): v is string => !!v)))
  ...
  const runDecisionTree = (answers: typeof onboardingAnswers) => {
    ...
    const matches = [...scoreMap.entries()].sort((a, b) => b[1] - a[1]).map(([id]) => normalizedOpportunities.find(o => o.id === id)!).slice(0, 6)
    return matches
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "signIn" && formData.email === TEMP_ADMIN.email && formData.password === TEMP_ADMIN.password) {
      ...
      if (!tempUser) {
        tempUser = { id: Date.now(), name: TEMP_ADMIN.name, email: TEMP_ADMIN.email, password: TEMP_ADMIN.password, userType: "admin" }
    ...
    if (mode === "signUp") {
      ...
      const newUser: User = { id: Date.now(), name: formData.name, email: formData.email, password: formData.password, userType, schoolName: userType === "school" ? formData.schoolName : undefined }
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20 flex items-center justify-center p-4">
      ...
      <Card className={cn("max-w-md w-full transition-all duration-500 ease-out border-none shadow-2xl", "bg-background/95 backdrop-blur-xl")}>
        ...
        <CardContent className="space-y-6">
            ...
            <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/40 p-4">
              <div className="w-full max-w-md bg-background rounded-xl p-6 shadow-lg">
                ...
                <div className="mt-4 flex gap-2 justify-end">
                  <Button variant="outline" onClick={() => { setShowPostSignup(false); router.push("/") }}>Later</Button>
                  <Button onClick={() => { setShowPostSignup(false); setShowOnboarding(true) }}>Okay</Button>
                </div>
              </div>
            </div>
          )}
          ...
          {showOnboarding && (
            <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/40 p-4">
              <div className="w-full max-w-lg bg-background rounded-xl p-6 shadow-lg">
                <h3 className="text-lg font-semibold mb-2">Quick Onboarding</h3>
                <p className="text-sm text-muted-foreground mb-4">Answer 3 quick questions to get tailored volunteer suggestions.</p>
                <div className="grid grid-cols-1 gap-3">
                  <CustomSelect options={[""].concat(categories)} value={onboardingAnswers.category} onChange={(v) => setOnboardingAnswers(prev => ({ ...prev, category: v }))} placeholder="Any category" />
                  <CustomSelect options={["Remote"].concat(emirates)} value={onboardingAnswers.emirate} onChange={(v) => setOnboardingAnswers(prev => ({ ...prev, emirate: v }))} placeholder="Any emirate" />
                  <CustomSelect options={[""].concat(skills)} value={onboardingAnswers.skill} onChange={(v) => setOnboardingAnswers(prev => ({ ...prev, skill: v }))} placeholder="Any skill" />
                </div>
                <div className="mt-4 flex gap-2">
                  <Button variant="outline" onClick={() => { setShowOnboarding(false); router.push("/") }}>Skip</Button>
                  <Button onClick={finishOnboarding}>Show suggestions</Button>
                </div>
                {suggestions.length > 0 && (
                  <div className="mt-4">
                    <h4 className="font-medium">Suggested opportunities</h4>
                    <ul className="mt-2 space-y-2 max-h-44 overflow-auto">
                      {suggestions.map(s => (
                        <li key={s.id} className="p-2 rounded-md border bg-muted/20">
                          <div className="font-semibold">{s.title}</div>
                          <div className="text-xs text-muted-foreground">{s.organization} • {s.emirate}</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === "signUp" && (
              <div className="relative group">
                ...
                <Input placeholder="Full Name" value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))} required className="pl-11 h-12 bg-muted/50 border-none focus-visible:ring-2 focus-visible:ring-primary/20 transition-all" disabled={isSubmitting} />
              </div>
            )}

            {mode === "signUp" && userType === "school" && (
              <select className="w-full h-12 bg-muted/50 border-none pl-3 rounded-lg focus-visible:ring-2 focus-visible:ring-primary/20 transition-all" value={formData.schoolName} onChange={(e) => setFormData(prev => ({ ...prev, schoolName: e.target.value }))} disabled={isSubmitting}>
                {schoolOptions.map((school) => <option key={school} value={school}>{school}</option>)}
              </select>
            )}

            {mode === "signUp" && userType === "ngo" && (
              <Input placeholder="NGO Name" value={formData.schoolName} onChange={(e) => setFormData(prev => ({ ...prev, schoolName: e.target.value }))} required className="w-full h-12 bg-muted/50 border-none pl-3 focus-visible:ring-2 focus-visible:ring-primary/20 transition-all" disabled={isSubmitting} />
            )}

            <div className="relative group">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
              <Input type="email" placeholder="Email address" value={formData.email} onChange={(e) => setFormData(prev => ({ ...prev, email: e.target.value }))} required className="pl-11 h-12 bg-muted/50 border-none focus-visible:ring-2 focus-visible:ring-primary/20 transition-all" disabled={isSubmitting} />
            </div>

            <div className="relative group">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
              <Input type="password" placeholder="Password" value={formData.password} onChange={(e) => setFormData(prev => ({ ...prev, password: e.target.value }))} required className="pl-11 h-12 bg-muted/50 border-none focus-visible:ring-2 focus-visible:ring-primary/20 transition-all" disabled={isSubmitting} />
            </div>
